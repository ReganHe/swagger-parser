...
export class Operation {
    ...
    this.opt.parameters.forEach((p) => {
      if (p.in === 'body') {
        if (Type.isObjectType(p.type)) {
          ...
          type.merge(
            new ObjectType([new Definition('__rawBody', p.type, true)])
          );
        }
      } else {
        // 其它情况肯定是 ObjectType
        if (Type.isObjectType(p.type)) {
          ...
          throw new Error(
            `内部解析引擎问题！（非 body 类型的参数应该都是 ObjectType）`
          );
        }
      }
    });
  ...
  toBase(config: {
    ...
    if (config.docPrefix) {
      doc.push(
        `@see [线上文档](${config.docPrefix}/${this.opt.rawTag}/${this.opt.rawId})`
      );
    }
  ...
  private __operateSubTypeByPath(
    ignoreWarn: boolean,
    type: Type,
    targetPath: string,
    omit?: boolean
...
function parseTargetPath(targetPath: string) {
  ...
  if (indexes.length)
    throw new Error(`路径 "${targetPath}" 最后一个不能是数组`);
